...
class Product extends DBModel
{
    ...
    {
        $sql = "UPDATE products SET is_deleted='1'
                                    WHERE id='$this->id'";
        $statement = self::prepare($sql);
        $statement->execute();
        return true;
    ...
    {
        $db = Database::getInstance();
        $req = $db->query("SELECT * FROM products WHERE id = '$id' AND is_deleted = false");
        $item = $req->fetchAll()[0];
    ...
    public static function getProductsByCategory($category_id)
    {
        $list = [];
        $db = Database::getInstance();
        $req = $db->query("SELECT * FROM products WHERE category_id = '$category_id' AND is_deleted = false");
    ...
    public static function getProductsByKeyword($keyword)
    {
        $list = [];
        $db = Database::getInstance();
        $req = $db->query("SELECT * FROM products WHERE name LIKE '%$keyword%' AND is_deleted = false");
